...
{
    MenuItem* backItem = MenuItemFont::create("Back", CC_CALLBACK_0(BasicRUBELayer::goBack, this));
    MenuItem* reloadItem = MenuItemFont::create("Reload", CC_CALLBACK_0(BasicRUBELayer::loadWorld, this));
    m_menuLayer = CCMenu::create(backItem,reloadItem, NULL);
    m_menuLayer->alignItemsHorizontallyWithPadding(10);
...
{
    unscheduleUpdate();
    Director::getInstance()->replaceScene(ExamplesMenuLayer::createScene());
}
...
void BasicRUBELayer::loadWorld()
{
    ...
    if ( m_world ) {
        ...
        log("%s", errMsg.c_str()); //if this warning bothers you, turn off "Typecheck calls to printf/scanf" in the project build settings
}
...
Point BasicRUBELayer::worldToScreen(b2Vec2 worldPos)
{
    worldPos *= getScale();
    Point layerOffset = getPosition();
...
void BasicRUBELayer::onTouchesMoved(const std::vector<Touch*>& touches, Event *event)
{
    if( touches.size() > 1)
    {
        ...
        Point previousMidpoint = previousScreenPos0.getMidpoint(previousScreenPos1);
        Point currentMidpoint = screenPos0.getMidpoint(screenPos1);
...
// Standard Cocos2d method
void BasicRUBELayer::onTouchesEnded(const std::vector<Touch*>& touches, Event *unused_event)
{
