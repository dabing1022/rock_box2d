...
void ButtonRUBELayer::setButtonHighlighted(_buttonInfo* bi, bool tf)
{
    ...
    if ( tf && !bi->imageFile_hover.empty() ) {
        bi->sprite->setTexture( CCSprite::create(convertToFullPath(bi->imageFile_hover))->getTexture() );
        ...
        bi->sprite->setTexture( CCSprite::create(convertToFullPath(bi->imageFile_normal))->getTexture() );
