...
std::string RUBELayer::getFilename()
{
    return "images.json";
}
...
Point RUBELayer::initialWorldOffset()
{
    ...
    return Point( s.width * (475 / 1024.0), s.height * (397 / 768.0) );
}
...
void RUBELayer::afterLoadProcessing(b2dJson* json)
{
    ...
    for (int i = 0; i < b2dImages.size(); i++) {
        ...
        Sprite* sprite = Sprite::create(convertToFullPath(img->file));
        if ( !sprite )
            continue;
        ...
        imgInfo->center = Point(img->center.x, img->center.y);
...
void RUBELayer::removeBodyFromWorld(b2Body* body)
{
    ...
    std::vector<RUBEImageInfo*> imagesToRemove;
    for (auto it = m_imageInfos.begin(); it != m_imageInfos.end(); ++it) {
        RUBEImageInfo* imgInfo = *it;
