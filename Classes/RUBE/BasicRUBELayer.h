...
protected:
    ...
    Touch* m_mouseJointTouch;    // keep track of which touch started the mouse joint

    Menu* m_menuLayer;           // only for this demo project, you can remove this in your own app
...
public:
    ...
    inline std::string convertToFullPath(std::string path)
    {
        size_t pos = path.find_last_of('/');
        std::string fullPath;
        if (pos != std::string::npos)
        {
            fullPath.assign(path, pos+1, std::string::npos);
        }
        else
        {
            fullPath = path;
    ...
    b2Fixture* getTouchedFixture(Touch* touch);      // returns the first fixture found under the touch location
