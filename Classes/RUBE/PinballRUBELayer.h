...
protected:
    ...
    Touch* m_leftFlipperTouch;                   // used to keep track of which flipper buttons are currently pressed (if this is nil, the button is not pressed)
    Touch* m_rightFlipperTouch;                  // used to keep track of which flipper buttons are currently pressed (if this is nil, the button is not pressed)
...
public:
    static Scene* scene();                       // returns a scene that contains this as the only child
    
    virtual std::string getFilename();                      // overrides base class
    virtual Point initialWorldOffset();          // overrides base class
