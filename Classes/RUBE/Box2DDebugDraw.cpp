...
void Box2DDebugDraw::DrawSolidPolygon(const b2Vec2* aVertices, int32 aVertexCount, const b2Color& aColor)
{
    ...
    DrawPrimitives::drawSolidPoly(mVertices, aVertexCount, Color4F(aColor.r, aColor.g, aColor.b, 0.5));
		...
		const b2Color& aColor)
{
    ...
    DrawPrimitives::drawSolidPoly(mVertices, DEBUG_DRAW_CIRCLE_SEGMENTS, Color4F(aColor.r, aColor.g, aColor.b, 0.5));
...
{
    DrawPrimitives::setDrawColor4F(aColor.r, aColor.g, aColor.b, 1);
    DrawPrimitives::drawLine(Point(aP1.x, aP1.y), Point(aP2.x, aP2.y));
}
