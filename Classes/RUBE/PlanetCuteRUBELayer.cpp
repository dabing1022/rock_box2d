...
void PlanetCuteRUBELayer::afterLoadProcessing(b2dJson* json)
{
    ...
    SimpleAudioEngine::getInstance()->preloadEffect("pickupgem.wav");
    SimpleAudioEngine::getInstance()->preloadEffect("pickupstar.wav");
...
{
    SimpleAudioEngine::getInstance()->unloadEffect("jump.wav");
    SimpleAudioEngine::getInstance()->unloadEffect("pickupgem.wav");
    SimpleAudioEngine::getInstance()->unloadEffect("pickupstar.wav");
...
void PlanetCuteRUBELayer::update(float dt)
{
    ...
    for (auto it = m_pickupsToProcess.begin(); it != m_pickupsToProcess.end(); ++it) {
        PlanetCuteFixtureUserData* fud = *it;
        
        // play sound effect
        if ( fud->pickupType == PT_GEM )
            SimpleAudioEngine::getInstance()->playEffect("pickupgem.wav", false);
        else if ( fud->pickupType == PT_STAR ) {
            SimpleAudioEngine::getInstance()->playEffect("pickupstar.wav", false);
    ...
    Size s = Director::getInstance()->getWinSize();
    Point sceneOffset = Point(s.width/2 - cameraCenterInPixels.x, s.height/2 - cameraCenterInPixels.y);
